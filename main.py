...
last_requests = [time.time(), time.time()]
...
def main():
    ...
    try:
        ...
        if event.type in [VkBotEventType.MESSAGE_NEW, VkBotEventType.MESSAGE_EVENT]:
            ...
            try:
                if (id < 2*10**9 and (states[id] == '{"command":"start"}' or
                                      any([cmd in text.lower().split()[0]
                                           for cmd in ('начать', 'start', 'ptbot', 'поехали', 'клавиатур')]))
                        and states[id] not in ['sending idea', 'sending question', 'sending partner']):
                    msg(id,
...
#team — вопросы к команде и о команде''',
                    ...
                    states[id] = '{"command":"start"}'
                      ...
                      (any([cmd in text.lower().split()[0]
                            for cmd in ('начать', 'start', 'ptbot', 'поехали', 'клавиатур', 'эй')]) or
                       any([cmd in text.lower().split()[1]
                            for cmd in ('начать', 'start', 'ptbot', 'поехали', 'клавиатур', 'эй')]))):
                    msg(id, 'Привет, я PTBot — чат-бот команды PTCodding. &#9995; '
                            'С моей помощью вы можете узнать последние новости и задонатить на топовый функционал '
                            'моим создателям — @ptcodding (команде PTCodding). Если вдруг я стану не нужен, напишите '
                            '«увидимся», «пока» или что-нибудь в этом роде.',
            ...
            if 'пинг' in text.lower().split()[:2]:
                ...
                if states[id] == '{"command":"partnership"}':
                    msg(id,
                        '''Станьте партнёрами PTCodding!
 Отправьте заявку сюда, и моё начальство рассмотрит её в ближайшие сроки. Следуйте моим советам:
...
Ответить пользователю: https://vk.com/gim132868814?sel={0}'''.format(
                    ...
                    msg(id,
                        '''Задать вопрос команде PTCodding на любую тему можно здесь! &#128521;
Также почитайте немного о команде: как появились они, как появился я, кто когда пришёл и какой вклад внёс &#128526;''',
                        keyboards.team)
...
Ответить пользователю: https://vk.com/gim132868814?sel={0}'''.format(
                    ...
                    msg(id,
                        '''PTCodding появился 11 ноября 2016 года. \
До 1 сентября 2018 года мы писали текстовые игры и много других проектов. С 1 сентября начали пиар-кампанию и активную\
 работу в медиа. Из известных работ: промо-ролик о митапе VK×Junction, подкасты «Взгляд в неделю» и «Шпион технологий».\
 2 февраля 2019 года переехали на платформы подкастов Anchor и ВКонтакте и начали активно распространяться на других. \
С марта по май в PTCodding публиковался экспериментальный подкаст «Игродайджест». В мае появился подкаст «IT-кухня». \
11 ноября 2020 года к своему четырёхлетию провели редизайн всего PTCodding. \
20 марта 2021 года переехали с Anchor на mave — русскоязычный хостинг подкастов.

Сейчас PTCodding — команда подростков из Санкт-Петербурга. Мы делаем интересный IT-контент и востребованные проекты, \
каждый день популяризуя программирование и технологии в массы. Специально для вас: подкасты с нашим мнением о \
недельных новостях, с интервью и свободными беседами, а также посты с самыми важными новостями.
...
Вы можете подробнее почитать о каждом участнике команды ниже, нажав на кнопку с именем участника! &#128071;''',
                    ...
                    msg(id, 'Павел — основатель PTCodding. Именно под его началом были созданы многие проекты, '
                            'концепт и окружение бренда. Павел является ведущим подкастов PTCodding и шеф-кодером '
                            'в команде программистов. Думает над дальнейшим продвижением и мыслит неординарно, '
                            'чтобы PTCodding становился лучше для своих слушателей и читателей. '
                            'Одним из крупнейших проектов Павла являюсь я — PTBot, чат-бот PTCodding!')

                elif states[id] == '{"command":"Bogdan"}':
                    msg(id, 'Богдан — дизайнер PTCodding. Присоединился к команде 14 сентября 2017 года. '
                            'Под его началом были сделаны все аватарки, шапки сообщества и обложкb подкастов. '
                            'Окружение бренда, дизайн и визуалка были сделаны его руками. '
                            'Является соведущим подкаста «IT-кухня», некоторое время вёл рубрику «Бюдгаджет месяца». '
                            'Одним из последних проектов стал редизайн товаров в магазине PTCodding.')

                elif states[id] == '{"command":"Dima"}':
                    msg(id, 'Дима — генератор идей команды PTCodding, главный редактор. '
                            'Присоединился к команде 5 мая 2020 года. Нарисовал характеристики к «Бюдгаджету месяца». '
                            'Ищет новых партнёров и достойных людей для сотрудничества и работы с командой PTCodding.')
                    ...
                    msg(id,
                        '''В разделе покупок Вы можете приобрести у нас виртуальные услуги и услуги на дом — рекламу, \
программирование и дизайн от моего начальства, звукозапись и ремонт Вашего ПК или Android-смартфона. &#128230;
...
Моё начальство напишет чат-бота ВКонтакте или в Telegram под Ваши цели и по Вашим требованиям. Ваши данные: описание \
функционала и ключевых целей и задач чат-бота, сгенерированный longpoll-токен.
Хочу предупредить Вас: если Вам не понравился чат-бот, есть какие-то проблемы, ошибки, баги, то необходимо сразу же \
сообщить об этом. Любые недочёты исправляются одним нажатием клавиши.
В чат-боте прописан его создатель, с указанием авторства на сообщество PTCodding. Просьбы вернуть деньги после того, \
как Вы установили чат-бота в своё сообщество, не принимаются.
Цена может изменяться в зависимости от срока заказа, указана минимальная цена! \
Также могут появляться специальные условия!
...
Моё начальство создаст дизайн Вашему бренду по Вашим требованиям. Ваши данные: название бренда, для которого делается /
дизайн, стиль исполнения.
Хочу предупредить Вас: если Вам не понравился какой-то элемент дизайна, то ему необходимо сразу написать об этом. Любые/
 недочёты исправляются одним нажатием клавиши.
На весь пак дизайна, до оплаты заказа, будет нанесён водяной знак. Просьбы вернуть деньги после того, как Вы оплатили /
заказ и пак был передан без водяных знаков, не принимаются.
Цена может изменяться в зависимости от срока заказа и техники выполнения, указана минимальная цена! Также могут /
появляться специальные условия!
...
Моё начальство запишет Ваш голос на своей домашней студии, обработает звук и отправит исходники в mp3, wav или другом \
формате на флешку, ВКонтакте или в облако.
При заказе дополнительно сведения, мастеринга или же при полноценной запись под ключ цена оговаривается при оформлении \
заказа.
Хочу предупредить Вас: на весь звуковой файл накладывается демо-фраза до оплаты заказа. Просьбы вернуть деньги после \
того, как Вы оплатили заказ и файл был передан без демо-фразы, не принимаются.
Цена может изменяться в зависимости от срока заказа и дополнительных услуг, указана минимальная цена! Для клиентов \
могут появляться специальные условия!
...
Моё начальство также работает с компьютерами и смартфонами: исправляет мелкие неполадки в работе, устанавливает \
лицензированный Microsoft Office или иное ПО, устанавливают активированную Windows, перепрошивают Android-смартфоны.
Хочу предупредить Вас: проверяйте работоспособность ПО, ПК или смартфона в целом сразу перед оплатой заказа. PTCodding \
даёт бесплатную гарантию — 1 месяц. Далее гарантия продлевается платно.
Цена может изменяться в зависимости от срока заказа, указана минимальная цена! Для клиентов могут появляться \
специальные условия!
...
Товары удаляются из корзины при нажатии на них. Если Вы добавили в корзину какие-то товары, то будет доступна кнопка \
оформления заказа — нажмите на неё, и с Вами в этом чате свяжутся мои операторы, которые обсудят все условия \
и цены &#128222;''',
                        keyboards.cartboard(id))

                elif states[id] == '{"command":"cart_ui"}':
                    msg(id, 'Загляните в привычный магазин, если мой Вам оказался не по душе: '
                        'vk.com/market-132868814 &#128072;')
                    ...
                    if id not in mute:
                        msg(admin_chat,
                            '''#order
Вас вызывают для оформления заказа. Пройдите, пожалуйста, по этой ссылке: \
https://vk.com/gim132868814?sel={}'''.format(id))
                        keyboards.carts[id] = []
                        msg(id, 'Ожидайте... Скоро мои операторы свяжутся с Вами для уточнения деталей '
                                'и оплаты заказа! &#8986;',
                            keyboards.cartboard(id))
...
Данные взяты из news.yandex.ru''',
                ...
                elif states[id] in ["{'command': '1'}", "{'command': '2'}", "{'command': '3'}", "{'command': '4'}",
...
Данные взяты из news.yandex.ru''',
                    ...
                    msg(id, 'Я очень хочу кушать. Я голодный... &#128546; Дайте, пожалуйста, пару долларов, чтобы мне '
                            'купили пончик &#127849;',
                    ...
                    mails[id] = False
                    if id < 2*10**9:
                        msg(id, 'Я отписал Вас от рассылок команды PTCodding. Если вдруг захотите подписаться обратно'
                                ' — ниже есть кнопка «Подписаться на рассылки» &#128071;',
                            keyboards.menu(mails[id]))
                    else:
                        msg(id, 'Я отписал этот чат от рассылок команды PTCodding. Если вдруг захотите подписаться'
                                ' обратно — ниже есть кнопка «Подписаться на рассылки» &#128071;',
                            keyboards.conversation(mails[id]))

                elif states[id] == '{"command":"mail"}':
                    mails[id] = True
                    if id < 2*10**9:
                        msg(id, 'Я подписал Вас на рассылки команды PTCodding. Если вдруг захотите отписаться — ниже '
                                'есть кнопка «Отписаться от рассылок» &#128071;',
                            keyboards.menu(mails[id]))
                    else:
                        msg(id, 'Я подписал этот чат на рассылки команды PTCodding. Если вдруг захотите отписаться — '
                                'ниже есть кнопка «Отписаться от рассылок» &#128071;',
                            keyboards.conversation(mails[id]))
...
#team — вопросы к команде и о команде''',
                ...
                elif id > 2*10**9 and ('исчез' in text.lower() or 'убер' in text.lower() or 'убр' in text.lower() or
                                       'скр' in text.lower() or 'пок' in text.lower() or 'св' in text.lower() or
                                       'увид' in text.lower() or 'уй' in text.lower() or 'уш' in text.lower()):
            ...
            if id == admin_chat:
                if state_chat == '{"command":"cancel"}':
                    ...
                    try:
                        msg(request_id, 'Меня попросили запросить у Вас оплату для «{}» на сумму в ₽{}. '
                            'Подтвердите оплату...'.format(text, request_amount),
                            keyboards.payboard(
                                'action=pay-to-group&amount={}&description={}&group_id=132868814&aid=10'.format(
...
С уважением, команда PTCodding.'''.format(name(restart_ids[i])['first_name']))
                        msg(restart_ids[i], 'Привет, это снова я, Ваш любимый PTBot! &#128075; Добро пожаловать в '
                            'старое доброе меню! &#128526;',
...
@{0} ({1} {2}) перевела ₽{3} с комментарием «{4}»'''.format(
                            ...
                            amount / 1000000, event.object.description))
                else:
                    msg(admin_chat,
                        '''#pay
...
&#128202; @{0} ({1} {2}) проголосовала в опросе по ссылке: vk.com/poll{3}_{4}'''.format(
                        domain, name(id)['first_name'], name(id)['last_name'],
                        event.object.owner_id, event.object.poll_id))
            elif id not in mute:
                msg(admin_chat,
                    '''#vote
...
&#127385; @{0} ({1} {2}) изменил полномочия участника команды PTCodding @{3} ({4} {5}) с «{6}» на «{7}»'''.format(
                    domain, name(id)['first_name'], name(id)['last_name'], change_domain,
                    name(change_id, 'gen')['first_name'], name(change_id, 'gen')['last_name'], level_old, level_new))
